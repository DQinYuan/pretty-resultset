package top.dqyuan.pretty_resultset;
...
public class PrettyPrinter {
    ...
    public static void printResultSet(ResultSet rs) throws SQLException {
        ...
        // 将 columnMaxLengths 每一项初始化成列名的长度
        ...
        while (iterator.hasNext()) {
            ...
            for (int i = 0; i < columnCount; i++) {
                System.out.printf("|%" + (columnMaxLengths[i] == 0? defaultLen: columnMaxLengths[i]) + "s", columnStr[i]);
    ...
    private static void printColumnName(ResultSetMetaData resultSetMetaData, int[] columnMaxLengths) throws SQLException {
        int columnCount = resultSetMetaData.getColumnCount();
        for (int i = 0; i < columnCount; i++) {
            System.out.printf("|%" + ((columnMaxLengths[i] == 0? defaultLen: columnMaxLengths[i])) + "s", resultSetMetaData.getColumnName(i + 1));
    ...
    private static void printSeparator(int[] columnMaxLengths) {
        for (int i = 0; i < columnMaxLengths.length; i++) {
            System.out.print("+");
            for (int j = 0; j < (columnMaxLengths[i] == 0? defaultLen: columnMaxLengths[i]); j++) {
